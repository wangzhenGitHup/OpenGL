...
int main()
{
	...
	Shader lamperShader("../shader/lamper.vert", "../shader/lamper.frag");
	...
	//create lamper VAO, VBO
	unsigned int lamperVAO, lamperVBO;
	glGenVertexArrays(1, &lamperVAO);
	glBindVertexArray(lamperVAO);

	glGenBuffers(1, &lamperVBO);
	glBindBuffer(GL_ARRAY_BUFFER, lamperVBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
	...
	//load textures
	unsigned int diffuseTexture = loadTexture("../image/container2.png");
	unsigned int specularTexture = loadTexture("../image/container2_specular.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightingShader.use();
		lightingShader.setFloat("material.shining", 64.0f);
		...
		lamperShader.setMat4("model", model);
		lamperShader.setMat4("view", view);
		lamperShader.setMat4("projection", projection);
